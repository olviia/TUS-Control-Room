	...
	public sealed partial class NdiReceiver : MonoBehaviour
	{
		...
		void PrepareReceiverObjects()
		{
			...
			if (_recv == null)
			{
				...
				// Only proceed with audio task setup if we successfully created a receiver
				if (_recv != null)
				{
				...
				{
					// If we couldn't create a receiver, don't try to set up the converter or property block
					return;
				}
			}
		...
		void ReceiveFrameTask()
		{
			try
			{
				...
				{
					Debug.LogError($"[NDI Receiver] Error capturing video frame: {e.Message}");
					// If we get an exception during capture, release and recreate the receiver next time
					ReleaseReceiverObjects();
				}
			}
