    ...
    public class PointCloudRenderer : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (fresh)
            {
                ...
                Debug.Log($"{Name()}: Fresh data - pointCount={pointCount}, rawPointSize={preparer.GetPointSize()}, pointSizeFactor={pointSizeFactor}, finalPointSize={pointSize}");
                ...
                if (pointSize == 0)
                {
                    pointSize = 0.01f * pointSizeFactor;
                    Debug.LogWarning($"{Name()}: pointSize == 0, using fallback size {pointSize}");
                }
                
                // Save the corrected point size to cache (after fallback)
            ...
            {
                // Use cached values from most recent reception
                pointCount = pointCountMostRecentReception;
                pointSize = pointSizeMostRecentReception;
            ...
            if (pointCount <= 0)
            {
                Debug.LogWarning($"{Name()}: pointCount is {pointCount}, skipping render");
                return;
            }
            
            if (pointSize <= 0)
            {
                Debug.LogError($"{Name()}: pointSize is {pointSize} after fallback, this should not happen!");
                return;
            }
#if xxxjack_silly
            ...
            Debug.Log($"{Name()}: Rendering {pointCount} points with size {pointSize} at position {transform.position}");
            
            // Use very large bounds to ensure the point cloud isn't culled
            Bounds renderBounds = new Bounds(transform.position, Vector3.one * 100);
            Graphics.DrawProcedural(material, renderBounds, MeshTopology.Points, pointCount, 1, null, block);
#if VRT_WITH_STATS
