            ...
            protected void run()
            {
                ...
                try
                {
                    while (!stopping)
                    {
                        ...
                        Debug.Log($"{Name()}: Received header: {hdrSize} bytes");
                        if (hdrSize != 16)
                        {
                        ...
                        Debug.Log($"{Name()}: FourCC received: 0x{fourccReceived:x}, expected: 0x{receiverInfo.fourcc:x}");
                        ...
                        Debug.Log($"{Name()}: Expecting {dataSize} bytes of data, timestamp: {timestamp}");
                        byte[] data = new byte[dataSize];
                        int actualDataSize = _ReceiveAll(socket, data);
                        Debug.Log($"{Name()}: Actually received {actualDataSize} bytes of data");
                        ...
                        Debug.Log($"{Name()}: About to enqueue packet to {receiverInfo.outQueue.Name()}");
                        bool ok = receiverInfo.outQueue.Enqueue(mc);
                        Debug.Log($"{Name()}: Enqueue result: {ok}");
#if VRT_WITH_STATS
...
#pragma warning disable CS0168
                ...
                {
                    Debug.LogError($"{Name()}: EXCEPTION IN TCP READER: {e.GetType().Name}: {e.Message}");
                    Debug.LogError($"{Name()}: Stack trace: {e.StackTrace}");
                    if (!stopping)
                    {
#if UNITY_EDITOR
                        throw;
