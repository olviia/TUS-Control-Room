...
namespace Cwipc
{
    ...
    public class NetworkPointCloudReader : AbstractPointCloudPreparer
    {
        ...
        private void Start()
        {
            ...
            InitReader();
            Debug.Log($"{Name()}: Start() completed, reader initialized");
        }
        ...
        {
            Debug.Log($"{Name()}: OnDestroy() called - WHY ARE WE BEING DESTROYED?");
            Debug.Log($"{Name()}: Stack trace: {System.Environment.StackTrace}");
            Stop();
